package com.apherox.redis.consumer;

import com.apherox.kafka.domain.Song;
import com.apherox.redis.service.SongService;
...
public class SimpleKafkaConsumer {
    ...
    @KafkaListener(id = "simple-kafka-music-consumer", topics = "music", groupId = "redis-song-consumer")
    public void consumeSong(Message<Song> message) {
        Objects.requireNonNull(message);
        Song song = message.getPayload();
        log.info("Redis consumer received Song: {}" + song);

        Context extractedContext = extractContext(message);

        try (Scope scope = extractedContext.makeCurrent()) {
            Span span = tracer.spanBuilder("redis-song-consumer")
